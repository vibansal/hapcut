...
// compact print
void print_fragment_vars1(FRAGMENT* fragment,VARIANT* varlist)
{
...
int generate_single_fragment(struct alignedread** readlist,FRAGMENT* flist,VARIANT* varlist,struct FOSMID* fosmid)
{
	...
	*/
	...
	{
		fprintf(stdout,"FRAGMENT_SINGLE "); print_fragment(&fp,varlist,stdout); 
		if (COMPARE_PHASE ==1) compare_fragment(&fp,varlist,stdout);
	}
	free(fp.alist); free(fp.id);
	print_fragment_vars1(&fragment,varlist); 
