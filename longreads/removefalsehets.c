	...
	for (i=0;i<snpfrag[v].frags;i++)
	{
		...
		for (t1=0;t1<Flist[j].blocks;t1++)
		{
			for (t2=0;t2<Flist[j].list[t1].len;t2++)
			{
				...
				if (t ==v) // t =v 
				{
					if (Flist[j].list[t1].hap[t2] == HAP1[t]) { Forig1 += prob1;  Forig2 += prob; LL_flip0 += prob; LL_flip1 += prob1; }
					else {  Forig1 += prob; Forig2 += prob1; LL_flip0 += prob1; LL_flip1 += prob; }
		...
		if (LL_flip0 > LL_flip1) L10 += LL_flip0 + log10(1.0+pow(10,LL_flip1-LL_flip0)); else L10 += LL_flip1 + log10(1.0+pow(10,LL_flip0-LL_flip1));
	}
	snpfrag[v].L00 = L00; snpfrag[v].L01 = L01; snpfrag[v].L11 = L11; snpfrag[v].Lnovar = Lnovar; snpfrag[v].L10 = L10; 
