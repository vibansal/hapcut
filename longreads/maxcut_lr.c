...
{
	int i=0,t=0,f=0;
	float oldLL= 0,newLL = 0; float scores[4]; float Ln,Lo,fL,chimeric_ll;
	// use the 4 values fragment.scores[] to calculate the likelihoods 
	for (i=0;i<component->frags;i++)
	{
		f = component->flist[i]; for (t=0;t<4;t++) scores[t] =  Flist[f].scores[t];
		//if (scores[0] > scores[1]) Lo = (scores[0] + log10(1+pow(10,scores[1]-scores[0]))); else Lo = (scores[1] + log10(1+pow(10,scores[0]-scores[1])));
		if (scores[2] > scores[3]) Ln = (scores[2] + log10(1+pow(10,scores[3]-scores[2]))); else Ln = (scores[3] + log10(1+pow(10,scores[2]-scores[3])));
		//oldLL += Lo; newLL += Ln;
		calculate_fragscore(Flist,f,hap,&fL,&chimeric_ll); oldLL += fL;
		newLL += Ln;
...
{
	int j=0,f=0,i=0,t=0,n=0,k=0,node=0,node1=0; float prob,prob1,prob2; float scores[4]; float Lo,Ln;
	...
	while (1)
	{
		...
		for (n=0;n<snpfrag[node1].frags;n++)
		{
			...
			for (j=0;j<Flist[f].blocks;j++) // first find the allele at 'node' in the fragment 'f' 
			{
				for (k=0;k<Flist[f].list[j].len;k++)
				{
					...
					if (node == startnode)
					{
						if (hap[node] == Flist[f].list[j].hap[k]) { Flist[f].scores[0] += prob2; Flist[f].scores[1] += prob; }
						else { Flist[f].scores[0] += prob; Flist[f].scores[1] += prob2; } 
						
						if (hap[node] == Flist[f].list[j].hap[k]) { Flist[f].scores[2] += prob2; Flist[f].scores[3] += prob; }
						else { Flist[f].scores[2] += prob; Flist[f].scores[3] += prob2; } 
					}
					else if (node == secondnode)  // probabilities are flipped compared to start node 
					{
						if (hap[node] == Flist[f].list[j].hap[k]) { Flist[f].scores[0] += prob; Flist[f].scores[1] += prob2; }
						else { Flist[f].scores[0] += prob2; Flist[f].scores[1] += prob; } 
						
						if (hap[node] == Flist[f].list[j].hap[k])  { Flist[f].scores[2] += prob; Flist[f].scores[3] += prob2; } 
						else { Flist[f].scores[2] += prob2; Flist[f].scores[3] += prob; } 
					}
				}
			}
			for (j=0;j<Flist[f].blocks;j++) // update score of every node outside 2 shores covered by 'f'
			{
				for (k=0;k<Flist[f].list[j].len;k++)
				{
					...
					if (node != startnode && node != secondnode)
					{
						...
						{ 
							scores[0] += prob2; scores[1] += prob;  // add node to startnode side of cut
							scores[2] += prob; scores[3] += prob2;  // add to other side of cut 
						} 
						else 
						{ 
							scores[0] += prob; scores[1] += prob2;  // allele mismatch so flip probability 
							scores[2] += prob2; scores[3] += prob; 
						}
		...
		if (node1 == startnode) 
		{
			node1 = secondnode;  if (node1 < 0) break; // if secondnode is not valid, we should exit, otherwise segfault
		}
...
void update_fragment_scores(struct SNPfrags* snpfrag,struct fragment* Flist,char* hap,int startnode,int secondnode,int node_added,struct PHEAP* pheap,int* slist)
{
	...
	float prob,prob1,prob2; float scores[4]; float Lo,Ln; float f_scores[4]; float oldscore;
		...
		for (j=0;j<Flist[f].blocks;j++) // update score of every node outside 2 shores covered by 'f'
		{
			for (k=0;k<Flist[f].list[j].len;k++)
			{
				...
				if (snpfrag[node].parent != startnode && snpfrag[node].parent != secondnode && node != node_added)
				{
					...
					if (hap[node] == Flist[f].list[j].hap[k]) { scores[0] += prob2; scores[2] += prob; scores[1] += prob; scores[3] += prob2; } 
					else { scores[2] += prob2; scores[0] += prob; scores[3] += prob; scores[1] += prob2; }
					if (scores[0] > scores[1]) Lo = (scores[0] + log10(1+pow(10,scores[1]-scores[0]))); else Lo = (scores[1] + log10(1+pow(10,scores[0]-scores[1])));
					if (scores[2] > scores[3]) Ln = (scores[2] + log10(1+pow(10,scores[3]-scores[2]))); else Ln = (scores[3] + log10(1+pow(10,scores[2]-scores[3])));
