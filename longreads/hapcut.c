...
int maxcut_haplotyping(char* fragmentfile,char* variantfile,int snps,char* outputfile,int maxiter_hapcut)
{
	...
	double MEM_ALLOC = 0; for (i=0;i<snps;i++) MEM_ALLOC += snpfrag[i].edges*0.002; MEM_ALLOC *= 0.008; 
...
int main(int argc, char** argv)
{
	...
	for (i=1;i<argc;i+=2)
	{
		...
		{
			NEW_CODE = atoi(argv[i+1]); SCORING_FUNCTION = 5; // for new code, scoring function is likelihood based and mec of fragment = -1*LL
