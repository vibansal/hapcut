/* 
 PROGRAM TO ESTIMATE HAPLOTYPE LENGTHS FROM SIMULATED SEQUENCE READS to evaluate impact of readlength, depth-of-coverage, insert-size
AUTHOR VIKAS BANSAL: first code June 4 2008, last modified July 10 2012, email: vbansal@scripps.edu
  */ 
...
// generate normally distributed random variable using Box-Muller method 
float normal_variable(float mean, float std)
{
	// theta = 2 * pi * rand()   || rho = sqrt(-2 x ln(1-rand())  || scale = stddev x rho  ||  x= mean + scale * cos(theta) || y = mean + scale * sin(theta) 
	float c = sqrt(-2*log(1.0-drand48()))*cos(2*M_PI*drand48()); 
	return c*std + mean;
}
...
int read_snplocs(char* snpfile,int MATES,int coverage,struct LIBRARY* libraries, int ls)  
{
	...
	if (sf == NULL) 
	{
		fprintf(stdout,"error opening variant file %s\n",snpfile); exit(0); 
	} 
	...
	int wsnps=0; uint64_t good_reads=0;
	...
	for (l=0;l<ls;l++)
	{
		...
		{
			start1 = lrand48()%(chromlength-libraries[l].readlength);  
			//if (start1 > chromlength-50000) start1 = start1-50000;
			//if (start1 > chromlength-libraries[l].mean-libraries[l].readlength) start1 = start1-libraries[l].mean-libraries[l].readlength;
			end1 = start1 + libraries[l].readlength;  // end1 should always be less than chromlength 
			...
			if (MATES > 0) // paired-end read 
			{
				boundary_flag = 0;
				start2 = end1 + (int)(normal_variable(libraries[l].mean,libraries[l].std)) - libraries[l].readlength; 
				end2 = start2 + libraries[l].readlength; 
				if (end2 >= chromlength) boundary_flag = 1; 

				if (boundary_flag ==1) { r--; continue; } // discard this read, decrement 'r' since read is not counted 
		...
		if (libraries[l].reads > 0) fprintf(stderr,"library %d RL:%d insertsize:%d/%d coverage %d #ofreads %ld hap-reads %ld fraction %0.3f\n",l+1,libraries[l].readlength,libraries[l].mean,libraries[l].std,libraries[l].coverage,libraries[l].reads,good_reads,(float)good_reads/libraries[l].reads);
	}
	...
	fprintf(stdout,"### totalcoverage %3d N50_vars %5d N50_span %6d N50_adjustedspan %6d",coverage,N50length_variants,N50length_span,N50length_adjustedspan);
	fprintf(stdout," components %6d phasedvariants %d/%d \t",components,variantGS-singletons,variantGS); 
	fprintf(stdout,"adjustedGS %f spanGS %f genomelen %d\n",adjustedGS,spanGS,chromlength);
	...
	*/
	if (remove(fragfile) == -1)  fprintf(stderr,"error in deleting fragment file %s \n",fragfile);
